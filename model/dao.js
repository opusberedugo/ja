...
class DAO {
  ...
  */
   ...
   * 
   * @param {string} userID 
   * @param {string} subjectNmae 
   * @returns {boolean} existState (if the note exists in the user profile [true for exist and false for not existing])
   */
  static subjectExist = async (userID , subjectName) => {
    const client = new MongoClient('mongodb://localhost:27017');
    ...
    try {
      ...
      const subject = await collection.findOne({ ownerID: new ObjectId(userID), subject: subjectName });
